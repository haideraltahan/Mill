...
```bash
mill eval meta-llama/Meta-Llama-3-8B-Instruct mmlu,mmlu_pro \
...
```bash
mill --output_dir ./results eval vllm mmlu \
  --model_args path=meta-llama/Meta-Llama-3-8B-Instruct,dtype=bfloat16
```

## API backend (LiteLLM)

Run any model accessible via an OpenAI-compatible API. API models support
**generative tasks only**, so use `mmlu_pro` (chain-of-thought), not the
log-prob `mmlu`:

```bash
# OpenAI
mill eval litellm mmlu_pro \
