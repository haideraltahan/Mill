...
Display aggregated performance and check for missing jobs. Results are read from
the long-format `aggregate.csv` (one row per `model, task, n_shot, metric` with
`performance` and `stderr` columns), so scores compare across any benchmark.
